...
namespace DataLayer
{
    public class ContactRepositorySP : IContactRepository
    {
        ...
        {
            using (var multiple = db.QueryMultiple("GetContact", new { Id = id }, commandType: CommandType.StoredProcedure))
            {
        ...
        {
            db.Execute("DeleteContact", new { id }, commandType: CommandType.StoredProcedure);
        }
